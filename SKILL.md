...
## 怎么用 / How to use
1. **判据 / criteria**：读 `reference/framework.md`（中文）或 `reference/framework.en.md`（English）。
2. **步骤 / steps**：严格按 `reference/workflow.md`（中文）或 `reference/workflow.en.md`（English）。
   按用户语言择一即可；两版等价。实例参考 `examples/`。
